...
### Typed routes lag behind new route files until the dev server runs
**Symptom:** `tsc --noEmit` rejects `router.push('/recurring')` right after creating
`src/app/recurring.tsx` — the string literal isn't in the route union.
**Cause:** expo-router's typed routes are codegen, not inference. The union in
`.expo/types/router.d.ts` is (re)generated by the dev server, so a fresh route file
doesn't typecheck until `expo start` has run once.
**Fix:** start (or restart) the dev server, then re-run `tsc`. In CI, generate types
first (`expo customize` won't do it; the server or `expo typegen`-equivalent step will).
...
- **Never commit a swipe-delete mid-drag.** Deleting while the finger is still down lets
  ...
  close the gap).
- **`Switch` ignores flexbox centering.** In a fixed-height row with
  `alignItems: 'center'`, the iOS `Switch` (UISwitch) rendered hugging the row's top edge
  (~2pt inset) while its text sibling centered normally — pixel-measured, not eyeballed.
  Its layout height evidently disagrees with what it paints. `alignSelf: 'center'` on the
  Switch itself fixes it.
